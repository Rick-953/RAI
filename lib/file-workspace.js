...
const SANDBOX_WORKSPACE_TTL_MS = 3 * 60 * 60 * 1000;
...
class FileWorkspace {
    constructor(options = {}) {
        ...
        this.ttlMs = boundedInteger(options.ttlMs, DEFAULT_TTL_MS, 60 * 1000, SANDBOX_WORKSPACE_TTL_MS);
    ...
    async _findReusableSandboxTask(owner) {
        let entries = [];
        try { entries = await fs.promises.readdir(this.rootDir, { withFileTypes: true }); } catch (_) { return null; }
        const now = this.now();
        for (const entry of entries) {
            if (!entry.isDirectory() || entry.isSymbolicLink() || !TASK_ID_RE.test(entry.name)) continue;
            try {
                const task = await this._readManifest(entry.name);
                if (
                    task.manifest.operation === 'sandbox_exec'
                    && task.manifest.userId === owner.userId
                    && now < Number(task.manifest.expiresAt)
                ) {
                    return task;
                }
            } catch (_) {
                // Stale or malformed task is left for cleanupExpired; never reuse it.
            }
        }
        return null;
    }

    async _refreshSandboxTask(task, owner) {
        task.manifest.sessionId = owner.sessionId;
        task.manifest.expiresAt = this.now() + SANDBOX_WORKSPACE_TTL_MS;
    ...
    async _writeArtifact(task, format, content, displayName) {
        ...
        const artifact = {
            ...
            size: bytes.length,
            sessionId: task.manifest.sessionId
        };
        ...
        return {
            ...
            downloadPath: `/api/file-jobs/${task.taskId}/artifacts/${artifactId}?sessionId=${encodeURIComponent(artifact.sessionId)}`
        };
    }
    ...
    async createSandboxTask({ userId, sessionId }) {
        ...
        const run = this.creationTail
            ...
            .then(async () => {
                await this.cleanupExpired();
                const reusable = await this._findReusableSandboxTask(owner);
                if (reusable) {
                    return await this._refreshSandboxTask({
                        taskId: reusable.manifest.taskId,
                        taskPath: reusable.taskPath,
                        manifest: reusable.manifest
                    }, owner);
                }
                const task = await this._createTaskUnlocked(owner, 'sandbox_exec');
                task.manifest.expiresAt = this.now() + SANDBOX_WORKSPACE_TTL_MS;
                await this._writeManifest(task.taskPath, task.manifest);
                return task;
            });
    ...
    async prepareDownload({ userId, sessionId, taskId, artifactId }) {
        ...
        const now = this.now();
        if (manifest.userId !== owner.userId) {
        ...
        const artifactSessionId = String(artifact?.sessionId || manifest.sessionId || '');
        if (artifactSessionId !== owner.sessionId) {
            throw new FileWorkspaceError('workspace_artifact_forbidden', 'workspace_artifact_forbidden', 404);
        }
        ...
        const lockPath = assertInsideRoot(taskPath, path.join(taskPath, `.download-${artifact.artifactId}.lock`));
        let lockHandle;
        try {
        ...
        const sandboxPersistent = manifest.operation === 'sandbox_exec';
        const finalize = async () => {
            if (!sandboxPersistent) {
                await this._removeTask(taskPath);
                return;
            }
            // A sandbox artifact is one disposable export from the user's
            // three-hour workspace. Consume only the export; keep /workspace.
        ...
        try {
            ...
            if (!sandboxPersistent) await this._removeTask(taskPath);
            throw error;
        }
    }
    ...
    async cleanupExpired() {
        ...
        for (const entry of entries) {
            ...
            let lockStat = null;
            try {
                const lockEntries = await fs.promises.readdir(taskPath);
                lockStat = lockEntries
                    .filter((name) => /^\.download-[a-f0-9]{32}\.lock$/.test(name))
                    .map((name) => path.join(taskPath, name))
                    .map((lockPath) => fs.promises.lstat(lockPath).catch(() => null));
                const lockStats = await Promise.all(lockStat);
                lockStat = lockStats.find((stat) => stat && (now - stat.mtimeMs) < TASK_LOCK_GRACE_MS) || null;
            } catch (_) {}
            const lockFresh = Boolean(lockStat);
