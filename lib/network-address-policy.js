...
async function requestPinnedHttp(target, options = {}) {
    ...
    for (const address of target.addresses) {
        // 本机无 IPv6 路由。IPv6 connect 会立即 ENETUNREACH，且在 Node 的
        // lookupAndConnectMultiple 与 ClientRequest 的 socket 关联竞态下，该错误
        // 可能以未捕获 'error' 事件直接击穿进程（RAI 曾因此反复崩溃重启）。
        // 这里仅尝试 IPv4；IPv6-only 主机自然失败，由上层 catch 处理。
        if (Number(address.family) === 6) continue;
        try {
