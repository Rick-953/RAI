...
(function () {
  ...
  var JSON_SIG_RE = /"crf_signature"\s*:\s*"v1\.([^."]+)\.([A-Za-z0-9_-]+)\.(\d+)"/;
  ...
  function detectFormat(text) {
    return text.replace(/^\uFEFF/, '').trimStart().charAt(0) === '{' ? 'json' : 'crf';
  }
  ...
  function normalizeJson(text) {
    return text.replace(/\r\n?/g, '\n').split('\n')
      .filter(function (l) { return l.indexOf('"crf_signature"') === -1 && l.indexOf('"crf_verify"') === -1; })
      .join('\n');
  }
  ...
  function metaOf(text) {
    ...
    var meta = { title: grab('title'), model: grab('model'), source: grab('source') };
    if (detectFormat(text) === 'json') {
      try {
        var doc = JSON.parse(text);
        meta = { title: doc.title || null, model: doc.model || null, source: doc.source || null };
      } catch (e) { /* 保持空 */ }
    }
    return meta;
  }
  ...
  function verify(text) {
    var isJson = detectFormat(text) === 'json';
    var m = isJson ? text.match(JSON_SIG_RE) : text.match(SIG_RE);
    ...
    sha256Hex(isJson ? normalizeJson(text) : normalizeCrf(text)).then(function (hash) {
      var payload = hash + '.' + ts;
