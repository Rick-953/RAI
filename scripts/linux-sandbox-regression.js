...
assert.doesNotMatch(source, /'--unshare-all'/, 'sandbox must not disable outbound networking');
assert.match(source, /'--share-net'/, 'sandbox must share the host network namespace for public internet access');
assert.match(source, /'--unshare-user'/);
