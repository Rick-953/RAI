...
function testVersionConstants() {
  ...
  assert.match(app, /const\s+RAI_APP_VERSION\s*=\s*'0\.13\.11'\s*;/, 'RAI_APP_VERSION mismatch');
  assert.match(app, /const\s+RAI_BUILD_ID\s*=\s*'20260819-persisted-flow-dedupe-v01311-r1'\s*;/, 'RAI_BUILD_ID mismatch');
  assert.match(serviceWorker, /const\s+RAI_SW_VERSION\s*=\s*'0\.13\.11-20260819-persisted-flow-dedupe-v01311-r1'\s*;/, 'RAI_SW_VERSION mismatch');
  const markers = (index.match(/20260819-persisted-flow-dedupe-v01311-r1/g) || []).length;
  assert.ok(markers >= 15, `index.html must carry >= 15 build markers, got ${markers}`);
  assert.match(index, /v0\.13\.11/, 'index.html must show the matching app version');
  assert.equal(packageJson.version, '0.13.11', 'package.json version mismatch');
}
