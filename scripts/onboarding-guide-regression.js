...
function testVersionConstants() {
  ...
  assert.match(app, /const\s+RAI_BUILD_ID\s*=\s*'20260924-stream-finalize-layout-v01316-r6'\s*;/, 'RAI_BUILD_ID mismatch');
  assert.match(serviceWorker, /const\s+RAI_SW_VERSION\s*=\s*'0\.13\.16-20260924-stream-finalize-layout-v01316-r6'\s*;/, 'RAI_SW_VERSION mismatch');
  const markers = (index.match(/20260924-stream-finalize-layout-v01316-r6/g) || []).length;
