...
function testVersionConstants() {
  ...
  assert.match(app, /const\s+RAI_BUILD_ID\s*=\s*'20260818-stream-timeline-v0138-r1'\s*;/, 'RAI_BUILD_ID mismatch');
  assert.match(serviceWorker, /const\s+RAI_SW_VERSION\s*=\s*'0\.13\.8-20260818-stream-timeline-v0138-r1'\s*;/, 'RAI_SW_VERSION mismatch');
  const markers = (index.match(/20260818-stream-timeline-v0138-r1/g) || []).length;
